...
final class SimplifiedIfReturnFixer extends AbstractFixer
{
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        ...
        for ($ifIndex = $tokens->count() - 1; 0 <= $ifIndex; --$ifIndex) {
            ...
            $lastSemicolonIndex = array_pop($indicesToClear);
            ...
            $newTokens = [new Token([\T_RETURN, 'return']), new Token([\T_WHITESPACE, ' ']), $match['isNegative'] ? new Token('!') : new Token([\T_BOOL_CAST, '(bool)'])];
    ...
    private function matchReturnSequence(Tokens $tokens, int $start): ?array
    {
        ...
        if ($tokens[$start]->equals('{')) {
            ...
            \assert(null !== $start);
            // token must exist otherwise $tokens is not valid syntax
        }
        ...
        \assert(null !== $bool1);
        // token must exist otherwise $tokens is not a valid syntax
        ...
        $indices[] = $bool2;
        $indices[] = $semi2;
        return ['isNegative' => $isNegative, 'indices' => $indices];
    }
}
